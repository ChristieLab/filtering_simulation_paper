...
This document will work you through a basic variant calling pipeline with `samtools` and `GATK`, calling out specific locations where filtering choices are made. The following software packages are required:
...
* `run_genomedb.sh`: This will run `GATK`'s `GenomicsDBImport`

If you use these scripts for publication, please cite this paper for now. These scripts must be located in the working directory, and are available at: ENTER GITHUB REPO...
...
mem <- 4 # number of GB of ram to allow for alignment and calling jobs
temp_dir <- tempdir() # temporary directory for alignment and calling jobs
run_chrs <- 29 # which chromosomes will we run?
...
MQRankSum <- -12.5
ReadPosRankSum <- -8
first_pass_missingness <- 0.2
min_genotype_quality <- 13.0103
```
...
Next, we will make databases from the `gvcf` files we produced for each individual in the previous step. For this example, we will prepare to run this for each chr, but will actually only run the first chromosome to save some time and memory. If you wish to run everything, change the object `run_chrs` in the first chunk from `29` to `1:29`. First, we need to build a sample map and a bed file for each region:
...
fwrite(sample_map, "hapmap.txt", sep = "\t")
...
```{r run_GenotypeGVCFs}
for(i in 1:nrow(bed)){
  cmd4 <- paste0("bash run_GenomicsDBImport.sh ",
                "hapmap.txt ",
                paste0(bed$chr[q], ".bed"), " ",
                mem, " ",
                temp_dir, " ",
...
Now we will run a set of filters on our variant calls to produce our final set of genotypes. Specifically, we're going to first apply `GATK`'s [hard filters](https://gatk.broadinstitute.org/hc/en-us/articles/360035890471-Hard-filtering-germline-short-variants), select only bi-allelic SNPs, and then do a first, quick filter to remove loci sequenced in very few individuals. We use the
`run_VariantFiltration.sh` script to do this, which calls `GATK`'s `VariantFiltration` tool, `vcftools`, and `bcftools`.

This is where we use most of the filtering parameters that we set at the start of this document.

```{r hard_filters}
foreach(q = 1:nrow(SRA_info)) %dopar% {
  cmd5 <- paste0("bash run_VarinatFiltration.sh ", 
                paste0(SRA_info[q,3], ".bed.vcf"), " ", 
                "monarch_genome.fna.gz ",
                QD, " ",
                FS, " ",
                SOR, " ",
...
Our final VCF file will then be stored in files named `hard_filt_pass_[CHRNAME].bed.recode.vcf.gz`. We will use this script in the second notebook, `post_variant_notebook.rmd`.
