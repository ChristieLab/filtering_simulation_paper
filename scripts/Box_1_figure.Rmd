...
fst_files <- c(fst_files, list.files("../data/empirical/outfiles/", "^[a-z].+fst\\.txt", full.names = TRUE))
...
PA_ss <- ggplot(ss_pstats, 
  ...
  theme(axis.title = element_text(size = text.sizes[1]),
        ...
        strip.text = element_text(size = text.sizes[2]),
...
PA_contour <- ggplot(contour[statistic == "P[seg]"  & dataset %in% c("Stoneflies", "Mountain goats RAD"),],
  ...
  guides(linetype = guide_legend(title = "Data Source"),
         color = guide_legend(title = "log10(HWP)")) +
  xlab("MAF")
...
imiss_SI_normed <- ggplot(imiss_pstats, 
       aes(x = min_ind*100, y = zero_normed, 
           group = interaction(dataset, subfacet), 
           color = dataset, linetype = type)) + 
  geom_line(size = 1.5) +
  facet_wrap(~statistic, scales = "free_y", labeller = label_parsed, nrow = 4) +
...
lmiss_SI_normed <- ggplot(lmiss_pstats, 
       aes(x = min_loci*100, y = zero_normed, 
           group = interaction(dataset, subfacet), 
           color = dataset, linetype = type)) + 
  geom_line(size = 1.5) +
  facet_wrap(~statistic, scales = "free_y", labeller = label_parsed, nrow = 4) +
...
ggsave("../manuscript/final_figures/SI_figures/FS9_lmiss_SI_normed.jpg", lmiss_SI_normed, height = 11, width = 15)
ggsave("../manuscript/final_figures/SI_figures/FS10_lmiss_SI_raw.jpg", lmiss_SI_raw, height = 11, width = 15)
```
