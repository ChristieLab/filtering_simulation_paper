...
demo_plot <- ggplot(demos, aes(x = Generation, y = N, group = Demography)) +
  ...
  theme(axis.title = element_text(size = 20),
        ...
        legend.title = element_text(size = 20)) +
  xlab("Generation") +
  scale_y_continuous(limits = c(-1000,120000))
...
PB <-  ggdraw() +
  draw_plot(demo_plot) +
  draw_plot(sfs_netural, x = .2, y = .265, width = inset_size, height = inset_size/2) +
  draw_plot(sfs_bottle, x = .64, y = .205, width = inset_size, height = inset_size/2) +
  draw_plot(sfs_expansion, x = .64, y = .827, width = inset_size, height = inset_size/2)
...
d_7 <- readRDS("../data/simulated/snpR_RDS/0.00005_50_0.2_7.RDS")
```
...
```{r pcas}
datsets <- list(d_.1, d_1, d_7)
names(datsets) <- c(".1", "1", "7")
...
pcas <- lapply(datsets, function(x) plot_clusters(x, "pop", simplify_output = TRUE))
```
...
PC <- plot_grid(colored_manhattans[[1]], 
                pre_pcas_ready[[1]] + 
                  ggtitle("Pre-filter") + 
                ...
                post_pcas_ready[[1]]+ 
                  ggtitle("Post-filter") + 
                  theme(plot.title = element_text(size = 20)),
                colored_manhattans[[2]], pre_pcas_ready[[2]], post_pcas_ready[[2]],
                colored_manhattans[[3]], pre_pcas_ready[[3]], post_pcas_ready[[3]],
                nrow = 3, ncol = 3, axis = "tl", align = "h")
